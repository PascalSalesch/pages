...
export default async function main () {
  ...
  // build the pages
  cmd.execSync('npm install --omit=dev', { cwd: root, stdio: 'inherit' })
  ...
  try {
    ...
    cmd.execSync(`mkdir -p ${targetOutput}`, options)
    cmd.execSync(`rsync -a ${tmp}/${prefix ? `${prefix}/*` : '*'} ${targetOutput}`, options)
    ...
    console.error(err)
    process.exit(err.message.includes('Command failed: git commit') ? 0 : 1)
  }
